import chess
from PyQt5.QtWidgets import QGraphicsScene, QGraphicsRectItem, QGraphicsTextItem, QGraphicsEllipseItem
...
class BoardScene(QGraphicsScene):
    ...
    def _add_legal_move_highlight(self, square: chess.Square, is_capture: bool, square_size: float, orientation: chess.Color, color: QColor = QColor(0, 0, 0, 30)):
        ...
        if is_capture:
            ...
            hl = QGraphicsEllipseItem(margin, margin, square_size - 2*margin, square_size - 2*margin)
            pen = QPen(color)
            ...
            hl = QGraphicsEllipseItem(square_size/2 - radius, square_size/2 - radius, 2*radius, 2*radius)
    ...
    def _add_highlight(self, square: chess.Square, color: QColor, category: str, square_size: float, orientation: chess.Color):
        ...
        if old_board:
            for square in chess.SQUARES:
                ...
                if matched_item:
                    ...
                    if animation_config.enabled and fen_changed and square != instant_square and dist > 1:
                    ...
                    piece_item.setPos(self.get_square_pos(square, square_size, orientation))
    ...
    def get_square_pos(self, square: chess.Square, square_size: float, orientation: chess.Color) -> QPointF:
    ...
    def _update_positions(self, square_size: float, orientation: chess.Color, theme: dict = None):
        ...
        for i in range(8):
            file_char = chess.FILE_NAMES[i] if orientation == chess.WHITE else chess.FILE_NAMES[7 - i]
            ...
            f_label.setPos(i * square_size + square_size - rect_f.width() - 1, 
                           7 * square_size + square_size - rect_f.height() - 1)

            rank_char = chess.RANK_NAMES[i] if orientation == chess.BLACK else chess.RANK_NAMES[7 - i]
            r_label = self.rank_labels[i]
            r_label.setFont(font)
            is_light_white_rank = ((7-i) % 2 != 0)
            is_light_black_rank = ((7+i) % 2 != 0)
            is_light = is_light_white_rank if orientation == chess.WHITE else is_light_black_rank
